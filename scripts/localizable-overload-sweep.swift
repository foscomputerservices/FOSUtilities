...
/// the five SDK versions (the actual identity) plus the Xcode version, which
/// is informational ONLY and never participates in a comparison decision.
/// Deliberately no date and no timestamp — same SDKs in, byte-identical
/// files out.
struct GenerationStamp {
    ...
    let xcodeVersion: String // "26.5"
}

func makeGenerationStamp(sdks: [PlatformSDK]) throws -> GenerationStamp {
    let result = try runXcrun(["xcodebuild", "-version"])
    guard result.status == 0,
          let first = result.stdout.split(separator: "\n").first,
          first.hasPrefix("Xcode ")
    else {
        throw Failure("xcodebuild -version did not report an Xcode version")
    }
    return GenerationStamp(
        ...
        xcodeVersion: String(first.dropFirst("Xcode ".count))
    )
}
...
func renderGeneratedStamp(_ stamp: GenerationStamp) -> String {
    ...
    // SDKs: \(stamp.sdkLine)
    // Xcode \(stamp.xcodeVersion) (informational)
...
func renderManifest(
    ...
    lines.append("- Xcode \(stamp.xcodeVersion) (informational)")
...
func main(options: Options) throws -> Int32 {
    ...
    if options.filter != nil {
        ...
        let stamp = try makeGenerationStamp(sdks: sdks)
