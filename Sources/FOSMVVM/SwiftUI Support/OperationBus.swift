...
public final class SyncOperationBus<A> {
    /// The collection of registered operations, keyed by ``ViewModelId`` for automatic dedup.
    ///
    /// Re-registering with the same ``ViewModelId`` replaces the previous operation.
    /// Operations are not guaranteed to execute in any particular order.
    private var operations: [ViewModelId: (inout A) -> Void]

    /// Registers a new operation to be invoked when `invoke(_:)` is called.
    ///
    /// If an operation is already registered for the given ``ViewModelId``, it is replaced.
    /// This prevents duplicate registrations when SwiftUI views reappear.
    ///
    /// - Parameters:
    ///   - viewModel: The ``ViewModel`` whose ``ViewModelId`` keys this operation.
    ///     Re-registering with the same ``ViewModel`` replaces the previous operation.
    ///   - operation: A closure that accepts an `inout` parameter of type `A`.
    ...
    /// bus.addOperation(for: viewModel) { value in
    ...
    /// bus.addOperation(for: viewModel) { formData in
    ///     formData.name = "Alice"
    /// }
    ///
    /// var data = FormData()
    ...
    /// ```
    public func addOperation(for viewModel: some ViewModel, _ operation: @escaping (inout A) -> Void) {
        operations[viewModel.vmId] = operation
    }

    /// Invokes all registered operations with the provided mutable value.
    ///
    /// All operations are executed synchronously. Each operation can read or mutate
    /// the value, and subsequent operations will see the changes.
    ///
    /// - Note: Operations are not guaranteed to execute in any particular order.
    ///
    ...
    /// var data = FormData()
    /// bus.invoke(&data)
    /// print(data.count) // Prints 1
    /// ```
    public func invoke(_ value: inout A) {
        operations.values.forEach { $0(&value) }
    }

    /// Creates a new, empty synchronous operation bus.
    ///
    /// The bus is initialized with no registered operations. Use `addOperation(for:_:)`
...
@MainActor
public final class AsyncOperationBus<A: Sendable> {
    /// The collection of registered asynchronous operations, keyed by ``ViewModelId`` for automatic dedup.
    ///
    /// Re-registering with the same ``ViewModelId`` replaces the previous operation.
    /// Operations execute concurrently and are not guaranteed to run in any particular order.
    private var operations: [ViewModelId: @Sendable (A) async -> Void]
    ...
    ///   - viewModel: The ``ViewModel`` whose ``ViewModelId`` keys this operation.
    ///     Re-registering with the same ``ViewModel`` replaces the previous operation.
    ///   - operation: An async closure that accepts a value of type `A`.
    ...
    /// ```swift
    /// let bus = AsyncOperationBus<String>()
    /// bus.addOperation(for: viewModel) { message in
    ///     try await Task.sleep(for: .seconds(1))
    ///     print("Async: \(message)")
    /// }
    /// ```
    public func addOperation(for viewModel: some ViewModel, _ operation: @escaping @Sendable (A) async -> Void) {
        operations[viewModel.vmId] = operation
    }
