...
public extension Link where Label == Text {
    /// Localizable-accepting form of SwiftUI's `Link.init(_:destination:)`.
    ///
    /// ## Example
    ///
    /// ```swift
    ...
    /// Link(_:defaultValue:destination:)
    /// ```
    ///
    /// - Parameters:
    ///   - localizable: The ``Localizable`` to display.
    ///   - defaultValue: Fallback text used if localization did not complete.
    nonisolated init(_ localizable: some Localizable, defaultValue: String? = nil, destination: URL) {
