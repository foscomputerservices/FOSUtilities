...
public protocol ClientHostedViewModelFactory: ViewModelFactory where Context == ClientHostedModelFactoryContext<Request, AppState> {
    ...
    /// Synchronous factory method that creates the ViewModel from context without localization
    /// - Parameter context: The factory context containing appState and other dependencies
    /// - Returns: An unlocalized ViewModel instance
    /// - Note: This method is typically generated by the @ViewModel macro
    static func modelSync(context: ClientHostedModelFactoryContext<Request, AppState>) throws -> Self
}
...
public extension ClientHostedViewModelFactory where Self == Request.ResponseBody {
    static func model(
        ...
        // Call the synchronous helper directly - no Task.synchronous needed!
        let model: Self = try Self.modelSync(context: context)
